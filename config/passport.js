                ...
                process.nextTick(function () {
                    Apostador.findOne({'local.email': email}, function (err, apostador) {
                        ...
                        // all is well, return apostador
                            ...
                            apostador.save(function (err, apostadorFound) {
                                if (err) {
                                    return done(null, false, req.flash('loginMessage', 'Oops! something is wrong'));
                                }
                                return done(null, apostadorFound);
...
function generateToken(apostador) {
    return jwt.sign(apostador, config.TOKEN_SECRET, { expiresIn: 86400 });
}